...
describe("PropsTransformer", () => {
	...
	describe("transformProps", () => {
		...
		it("should register handler with unique ID", () => {
			...
			const handlerId = getHandlerId(result.onClick as { $handler: string });
		...
		it("should handle multiple event handlers", () => {
			...
			expect(getHandlerId(result.onClick as { $handler: string })).not.toBe(
				getHandlerId(result.onChange as { $handler: string }),
			);
		});
