...
describe("HandlerRegistry", () => {
	...
	describe("execute", () => {
		it("should execute sync handler and return action", async () => {
			const action: Action = { type: "CLICK", payload: { x: 100 } };
			const handler = () => action;
			const id = registerHandler("ext-1", handler);

			const event: SerializedEvent = { type: "click" };
		...
		it("should execute async handler and return action", async () => {
			const action: Action = { type: "ASYNC_CLICK" };
			const handler = async () => {
		...
		it("should throw error for non-existent handler", async () => {
			...
			await expect(executeHandler("nonexistent", event)).rejects.toThrow(
				"Handler not found: nonexistent",
			);
		});

		it("should pass event to handler", async () => {
			let receivedEvent: SerializedEvent | null = null;
			...
			const event: SerializedEvent = { type: "click", target: { id: "btn" } };
